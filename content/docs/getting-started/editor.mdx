...
After a **native build**, rustc errors are picked up by the **`$tish-rustc`** problem matcher contributed by the extension.
  ...
  configs.tish_lsp = {
    default_config = {
      ...
      root_dir = function(fname)
        return vim.fs.dirname(vim.fs.find({ 'zectre.yaml', 'package.json', '.git' }, { path = fname, upward = true })[1])
      end,
