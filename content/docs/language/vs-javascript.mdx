...
| `import` / `export` | Modules for builtins (`http`, etc.), native **`tish:*`** and **`cargo:…`** specs on the Rust native backend (`cargo:` uses `tish.rustDependencies` in `package.json`), and multi-file resolution where the toolchain supports it — not arbitrary npm package graphs on all targets (see below). |
...
Tish supports **`import`** and **`export`** for **language builtins** (e.g. `import { serve } from 'http'` with the `http` feature), **native module specs** such as **`tish:fs`**, **`tish:http`**, **`tish:process`**, **`tish:ws`**, etc., and **`cargo:crate_key`** imports backed by **`tish.rustDependencies`** on the **Rust native compile** path, plus **multi-file** programs where the resolver is enabled. **`tish:*`**, **`cargo:…`**, and npm-linked native modules require **`tish build --native-backend rust`** (not the Cranelift/LLVM “embedded bytecode only” binary paths). This is **not** “every npm package on every target”; dynamic **`import()`** / **`import.meta`** are out of scope for ECMA alignment in the same way as in the main Tish docs.
