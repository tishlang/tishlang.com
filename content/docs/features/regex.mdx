...
Requires the `regex` feature. Build tish with it for run: `cargo run -p tishlang--features regex -- run main.tish`. For build: `tish build main.tish -o app --feature regex`.
